...
$stat_array = array();
$sql = "
	SELECT COUNT(MSGKEY) cnt, DATE(`RSLTDATE`) AS `yyyymmdd`, ETC3 AS storeno, FILE_CNT AS fcnt
	FROM {$agent_name}
	WHERE RSLT = '1000'
		AND DATE(RSLTDATE) = '{$yyyymmdd}'
	GROUP BY `yyyymmdd`, storeno, fcnt
";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
    	$stat_array[$i]['kind'] = ($row['fcnt'] == '0' ? '1' : '2');
...
foreach ($stat_array as $val) {
	...
	";
	if ($val['kind'] == '1') $sql .= "lg_lms = '{$val['cnt']}' ";
	else $sql .= "lg_mms = '{$val['cnt']}' ";
	$sql .= "
		ON DUPLICATE KEY
		UPDATE
	";
