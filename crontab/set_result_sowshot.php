...
$table_name = 'sowshot_res';
$backup_table_name = 'sowshot_res_backup';
...
$sql = "
	SELECT
		etc1,
		SUM(CASE WHEN result IN (0, 1000) THEN 1 ELSE 0 END) AS success,
		SUM(CASE WHEN result NOT IN (0, 1000) THEN 1 ELSE 0 END) AS fail
	FROM {$table_name}
	WHERE etc4 = '{$time_flag}'
	GROUP BY etc1
";
...
$sql = "
	...
	SELECT
		...
		receiveNo,
		callbackNo,
		CASE WHEN result IN (0, 1000) THEN 0 ELSE result END,
		CASE WHEN telecom is NOT NULL THEN telecom ELSE 'ETC' END,
		sentTime,
		resultTime
	FROM {$table_name}
	WHERE etc4 = '{$time_flag}'
