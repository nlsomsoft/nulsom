...
$stat_array = array();
$cur_time = time();
$yyyy = (int)date('Y', strtotime('-1 month', $cur_time));
$mm = (int)date('m', strtotime('-1 month', $cur_time));
$dd = (int)date('d'); // 현재일
...
if ($dd >= 5) {
	$sql = "
		UPDATE stat_sending_month
		SET fix_flag = '1'
		WHERE yyyy = '{$yyyy}' AND mm = '{$mm}'
	";
	if (!mysqli_query($conn, $sql)) {
		mysqli_close($conn);
	    error_log('['.$log_name.'][E]...['.$sql.']', 0);
		exit;
	}
	error_log('['.$log_name.'][F]...['.$file_name.']', 0);
	exit;
