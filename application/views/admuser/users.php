...
<div class="local_ov01 local_ov">
    총회원수 <?=number_format($total_rows)?>명 중 <a href="#">정상 <?=number_format($total_rows1)?></a>명, <a href="#">발송제한 <?=number_format($total_rows2)?></a>명
    <?php if ((int)$this->session->userdata('level') >= 5) { ?>
    <span style="padding-left:30px;"> 총 보유잔액 : <?=number_format($cash_info['total'])?> 원, 총 정지회원잔액 : <?=number_format($cash_info['total_block'])?> 원</span>
    <?php } ?>
</div>
...
<?php
    ...
    echo form_open('admuser/users', $attributes);
...
<?php } ?>
    ...
    <span class="btn_add01">
        <a href="#userModal_channel" data-toggle="modal">채널변경(전체)</a>
    </span>
    <?php } ?>
</div>
<div class="tbl_head02 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" class="td_date">회원유형</th>
    ...
    <?php } ?>
        <th scope="col" class="td_date">가입일</th>
    <?php if ((int)$this->session->userdata('level') >= 5) { ?>
        <th scope="col" class="td_date">타입</th>
    <?php } ?>
        <th scope="col" class="td_date">발신자명</th>
    ...
    foreach ($result as $row) {
        ...
        if ($row->level == '0') {
            if ((int)$this->session->userdata('level') < 5) {
                $state = '<span style="color:#e8180c;">[승인대기]</span>';                
            } else {
                $state = '<a href="/admuser/login/'.$row->userno.'" style="text-decoration:none;"><span style="color:#e8180c;">[승인대기]</span></a>';                
            }
        } else {
            $state = '';
        ...
        else if ((int)$row->level == 9) $admin_flag = '<span style="color:#FF0000;">(S)</span>';
        else $admin_flag = '';
?>
    <tr class="bg<?=(int)($i%2)?>">
        ...
        <td headers="mb_list_name" class="td_date"><?=($row->user_type == '1' ? '개인' : '사업자')?></td>
    ...
    <?php } ?>
        <td headers="mb_list_name" class="td_date" style="width:140px;font-weight:bold;text-align:right;padding-right:5px;"><?=number_format($row->cash)?></td>
    <?php if ((int)$this->session->userdata('level') >= 5) { ?>
    ...
    <?php } ?>
        <td headers="mb_list_name" class="td_date"><span class="txt_true"><?=number_format($row->sms1,2)?></span></td>
    <?php if ((int)$this->session->userdata('level') >= 5) { ?>
        <td headers="mb_list_name" class="td_date"><?=$channel_info[$row->ch_sms]?></td>
    <?php } ?>
        <td headers="mb_list_name" class="td_date"><span class="txt_true"><?=number_format($row->lms1,2)?></span></td>
    <?php if ((int)$this->session->userdata('level') >= 5) { ?>
        <td headers="mb_list_name" class="td_date"><?=$channel_info[$row->ch_lms]?></td>
    <?php } ?>
        <td headers="mb_list_name" class="td_date"><span class="txt_true"><?=number_format($row->mms1,2)?></span></td>
    <?php if ((int)$this->session->userdata('level') >= 5) { ?>
    ...
    <?php } ?>
        <td headers="mb_list_name" class="td_date"><?=mydate_format('y-m-d', $row->add_date)?></td>
    <?php if ((int)$this->session->userdata('level') >= 5) { ?>
        <td headers="mb_list_name" class="td_date"><?=($row->ad_type == '1' ? '광고' : '일반')?></td>
    <?php } ?>
        <td headers="mb_list_name" class="td_date" style="width:120px; text-align:left;"><?=$row->ad_title?></td>
    ...
    <?php } else if ((int)$this->session->userdata('level') == 3) { ?>
...
<!-- The Modal Start -->
<div class="modal fade" id="userModal_channel">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">채널변경(전체)</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
    ...
    <?php
        $attributes = array(
            'name' => 'fadminchannel',
            'id' => 'fadminchannel',
            'onsubmit' => 'return admin_change_channel_submit(this);'
        );
        echo form_open('admuser/change_channel', $attributes);
    ?>
                <div class="form-group" style="text-align:center;">
    <?php
        $options = array();
        $options[''] = '채널 타입을 선택하세요.';
        $options['sms'] = '단문 채널';
        $options['lms'] = '장문 채널';
        $options['mms'] = '포토 채널';
        $js = 'class="frm_input" style="width:90%; height:40px;" onchange="getChannelList(this.value);"';
        echo form_dropdown('channel_type', $options, $groupno, $js);
    ?>
                </div>
                <div class="form-group" style="text-align:center;">
    <?php
        $options = array();
        $options[''] = '변경할 채널을 선택하세요.';
        $js = 'class="frm_input" style="width:90%; height:40px;"';
        echo form_dropdown('channel_dest', $options, $groupno, $js);
    ?>
                </div>
                <div class="form-group" style="text-align:center;">
    <?php
        $options = array();
        $options[''] = '새로운 채널을 선택하세요.';
        $js = 'class="frm_input" style="width:90%; height:40px;"';
        echo form_dropdown('channel_target', $options, $groupno, $js);
    ?>
                </div>
...
<script type="text/javascript">
var getChannelList = function (channel_type) {
    if (!channel_type) {
        $("input[name=channel_dest]").find("option").remove().end().append("<option value=''>변경할 채널을 선택하세요.</option>");
        $("input[name=channel_target]").find("option").remove().end().append("<option value=''>새로운 채널을 선택하세요.</option>");
        return;
    }
    ...
    $.ajax({
        ...
        success : function(data, status, xhr) {
            ...
            //SELECT BOX 초기화
            $("select[name=channel_dest]").find("option").remove().end().append("<option value=''>변경할 채널을 선택하세요.</option>");
            var added_option = '';
            //배열 개수 만큼 option 추가
            $.each(data.channel_dest, function(key,val){
                added_option = val.split('|');
                $("select[name=channel_dest]").append("<option value='"+added_option[0]+"'>"+added_option[1]+"</option>");
            });
            $("select[name=channel_target]").find("option").remove().end().append("<option value=''>새로운 채널을 선택하세요.</option>");
            $.each(data.channel_target, function(key,val){
                added_option = val.split('|');
                $("select[name=channel_target]").append("<option value='"+added_option[0]+"'>"+added_option[1]+"</option>");
            });
        },
...
var admin_change_channel_submit = function () {
    var channel_type = $("select[name=channel_type]").val();
    var channel_dest = $("select[name=channel_dest]").val();
    var channel_target = $("select[name=channel_target]").val();
    ...
    if (!confirm('채널을 변경하시겠습니까?')) return false;
    return true;
}
...
<div class="modal fade" id="accountModal">
    ...
    ?>
                ...
                <input type="radio" name="rd_bill_mode" value="PB" checked="checked" /> <span style="font-family:dotum; font-size:12px">무통장입금</span>
                </label>
                <label class="btn btn-sm">
                ...
                <div class="form-group">
                <input type="text" class="form-control" name="ipt_amount" placeholder="(필수) 금액입력" style="font-family:dotum; font-size:12px;">
                </div>
                <div class="form-group">
                <textarea class="form-control" name="ipt_memo" id="ipt_memo" placeholder="(선택) 관리메모" style="font-family:dotum; font-size:12px;"></textarea>
                </div>
...
var admin_billing_submit = function () {
    ...
    //     return false;
    // }
    var result = confirm("충전.차감 타입을 확인하세요. 계속 진행할가요?");
    if (!result){
        return false;
    }
    return true;
