...
<?php
    ...
    echo form_open('admstats/send_channel', $attributes);
    ...
    if ((int)$this->session->userdata('level') > 5) {
        ...
        if (is_array($channel_list)) {
            foreach ($channel_list as $row) {
                $options[$row->channel] = strtoupper($row->type).'('.$row->channel_exp.')';
            }
        }
        $js = 'class="frm_input" style="width:110px"';
        echo form_dropdown('ipt_channel', $options, $channel, $js);
    }
    if ((int)$this->session->userdata('level') > 5) {
        $options = array();
        $options[''] = '전체 (상점)';
        ...
        $js = 'class="frm_input" style="width:110px" onchange="getGroupList(this.value);"';
    ...
    if (is_array($product)) {
        foreach ($product as $row) {
            $options[$row->productcode] = $row->productname;
        }
    }
    $js = 'class="frm_input" style="width:110px"';
    echo form_dropdown('ipt_productcode', $options, $productcode, $js);
?>
...
?>
    <tr class="bg<?=(int)($i%2)?>">
        <td class="td_num" style="letter-spacing: 0.1em;"><?=convert_campaign_priority($row->priority)?></td>
        <td class="td_num" style="letter-spacing: 0.1em;"><?=number_format($row->total_units)?></td>
        ...
        <td class="td_num" style="letter-spacing: 0.1em;"><?=number_format($row->remain_units)?></td>
        <td class="td_num" style="letter-spacing: 0.1em;"><?=(int)((($row->success + $row->fail)/$row->total_units)*100).'%'?></td>
    </tr>
<?php
        $i ++;
    }
?>
    <tr class="bg1">
        ...
        <td class="td_num" style="letter-spacing: 0.1em;"><?=number_format($total_result->total_units)?></td>
        <td class="td_num" style="letter-spacing: 0.1em;"><?=number_format($total_result->success)?></td>
        <td class="td_num" style="letter-spacing: 0.1em;"><?=number_format($total_result->fail)?></td>
        <td class="td_num" style="letter-spacing: 0.1em;"><?=number_format($total_result->remain_units)?></td>
        <td class="td_num" style="letter-spacing: 0.1em;"><?=(int)((($total_result->success + $total_result->fail)/$total_result->total_units)*100).'%'?></td>
    </tr>
            </tbody>
    </table>
</div>
