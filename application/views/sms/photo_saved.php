...
<div class="tab_menu1">
    <ul>
    <li><a href="#" onclick="alert('단문.장문 문자보내기에서 이용 가능합니다.'); return false;">단문.장문</a></li>
    <li class="on"><a href="/sms/photo_saved"><font color="#fff">포토</font></a></li>
    </ul>
</div>
        ...
        <td align="center">
            ...
            <div class="btn2-group" style="padding-top:4px;padding-left:5px;">
              ...
              <?php
                    ...
                    $g_search_action = 'sms/photo_saved';
                        ...
                        <dl class="<?=($i == 2 ? 'last' : '')?>">
                            <dt>
                                <p>
                                    ...
                                    <label class="subject" for="mysms_2257480"><?=$row->subject?></label>
                                </p>
                            </dt>
                            <dd class="photo">
                                <div class="photo_area" onclick="setSavedMessage('<?=$row->smno?>');">
                                    <img class="photo_preview" id="photo_image_<?=$row->smno?>" alt="" src="<?=$row->photo_image?>" style="width:auto; max-width:215px; height:auto; max-height: 320px; padding-top:0; !important ">
                                </div>
                            </dd>
                            <dd class="textarea">
                                <textarea class="text" rows="5" cols="8" readonly id="msg_<?=$row->smno?>" onclick="setSavedMessage('<?=$row->smno?>');"><?=$row->msg?></textarea>
                            </dd>
                        <dd class="option">
                            <p class="date"><?=mydate_format('Y-m-d',$row->add_date)?></p>
                            <p class="byte"><span id="bytes_<?=$row->smno?>"><?=(int)$row->bytes?></span>byte</p>
                        </dd>
                        </dl>
                <?php
                        $i ++;
...
var setSavedMessage = function (smno) {
    ...
    var imgurl = $("#photo_image_"+smno).attr("src");
    $("#photo_image",opener.document).attr("src", imgurl);
...
var deleteSavedMessage = function () {
    ...
    $.ajax({
        ...
        success : function(data, status, xhr) {
            ...
            $(location).attr('href', '/sms/photo_saved');
        },
