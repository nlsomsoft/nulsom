		...
		<td width="210" valign="top">
			...
			<td width="960" valign="top">
				...
				<div class="content_wrap">
					<div class="body_title" style="position:relative;">내용바꿔 <img src="/images/vote_icon.png">
						<div class="faq_sch">
...
echo form_open('send/send_switch', $attributes);
?>
<?php
													...
													<?php
														if ($svc == 'elect') {
															if ($this->session->userdata('phone_080')) {
																$vote_bottom_msg1 = '무료거부 '.$this->session->userdata('phone_080');
												...
												<div class="phone_bg">
													<div class="phone_bg_inner msg_sub_color2" style="background-color:#deeaf5;">
														...
														<div class="msg_sub_color2" style="margin-top:10px;">
															<input id="votemsgtop2" name="vote_msg_top" type="text" class="phone_bg_msg" style="display:none;font-weight:600;font-size:13px;color:#4576c7;" value="(선거운동정보)" readonly />
...
<table width="100%" border="1" style="border: 1px solid #bbb; margin-top:30px" cellpadding="0" cellspacing="1" bgcolor="#fff" class="class_tbl">
								...
								<?php } ?>
									...
									<div class="bot_btn" style="margin-top:20px;margin-bottom:40px;">
 ...
 <div style="font-size:12px; font-family:'굴림','굴림체','Arial'; margin-bottom:30px">

 	<table border="0" cellspacing="0" cellpadding="0">
 		<tr>
 			<td style="padding:0 5px;">
 				<div style="border:1px solid #ccc; padding-bottom:10px;margin-top:-12px;">
				<select class="send-targets input_261" name="send_targets" multiple style="width:370px;height:310px; line-height:14px;">
<?php
...
?>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td style="padding:0 5px;"><strong>총 <span id="target_count" style="color:#ec4000;font-weight:bold;"><?=count($cached_remain_array)?></span> 명</strong></td>
						<td style="padding:0 5px;" align="right"><div class="style_btn" style="width:50px;"><a href="#" onclick="removeItem(); return false;">삭제</a></div></td>
					</tr>
					<tr>
						<td colspan="2">
              <div class="btn1-group" style="padding-top:4px;padding-left:5px;">
                <div class="btn1-black font-13" style="border:0px;width:175px;text-align:center;">전체건수</div>
                <input id="total_count" type="text" class="input_261 input-elect" title="전체건수" readonly value="<?=number_format($tcount)?>">
              </div>
              <div class="btn1-group" style="padding-top:4px;padding-left:5px;">
                <div class="btn1-black font-13" style="border:0px;width:175px;text-align:center;">전송건수</div>
                <input id="send_count" type="text" class="input_261 input-elect" title="전송건수" readonly value="<?=number_format($tcount - $rcount)?>">
              </div>
              <div class="btn1-group" style="padding-top:4px;padding-left:5px;">
                <div class="btn1-black font-13" style="border:0px;width:175px;text-align:center;">잔여건수</div>
                <input id="remain_count" type="text" class="input_261 input-elect" title="잔여건수" readonly value="<?=number_format($rcount)?>">
              </div>
              <div class="btn1-group" style="padding-top:4px;padding-left:5px;">
                <a href="#" onclick="initElectList();"><div class="btn1-blue font-13" style="border:0px;width:175px;text-align:center;">발송목록초기화</div></a>
              </div>
              <div class="btn1-group" style="padding-top:4px;padding-left:5px;">
                <a href="/sms/sms_excel"><div class="btn1-blue font-13" style="border:0px;width:175px;text-align:center;">잔여목록다운로드</div></a>
              </div>
						</td>
					</tr>
				</table>
...
<?php if ($this->session->userdata('callback') == false) { ?>
	...
	layerOpenLocationNew("c_layer", _a[0] + 431, _a[1] + 81);
<?php } ?>
...
function vote_byte() {
	...
	if (svc == 'elect') {
		...
		if (phone_ext == 0) return (50 + phone_080);
		else return (55 + phone_080 + phone_ext);
	} else {
...
var resultProcess = function() {
	...
	setTimeout("layerCloseNew('result_layer')", 1500);
}
var resultProcess = function() {
	var _target = document.getElementById('result_layer');
	var _location_base = document.getElementById("base_1");
...
var hotSettings = {
	data: dataObject,
	columns: [{data:'var1',type:'text'},{data:'var2',type:'text'},{data:'var3',type:'text'},{data:'var4',type:'text'},{data:'var5',type:'text'}],
	stretchH: 'all',
	width: 590,
	height: 500,
	minSpareRows: 20,
...
buttons.string.addEventListener('click', function() {
	...
	for (var i = 0; i < 10; i++) {
		org_msg = org_msg.replace( /\[\*n\*\]/, lines[0] );
		org_msg = org_msg.replace( /\[\*1\*\]/, lines[1] );
		org_msg = org_msg.replace( /\[\*2\*\]/, lines[2] );
		org_msg = org_msg.replace( /\[\*3\*\]/, lines[3] );
		org_msg = org_msg.replace( /\[\*4\*\]/, lines[4] );
	}
...
buttons.string2.addEventListener('click', function() {
	...
	registerMessages(excel_list,lms_notice);
});

var _send_lock = false;
function registerMessages(excel_list,lms_notice) {
	if (_send_lock != false) {
	...
	_send_lock = true;
	form.excel_list.value = excel_list;
...
var initElectList = function() {
  ...
  $.ajax({
      ...
      success : function(data, status, xhr) {
      	alert(data.message);
      	$(location).attr('href', '/sms/newswitch');
      },
