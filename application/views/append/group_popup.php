        ...
        <td align="center">
            <div class="body_title" style="padding-left:10px;position:relative;">
              <div class="btn2-group" style="padding-top:4px;padding-left:5px;">
                <a href="/append/group_popup" class="btn1-black btn1-space"><div style="width:80px;text-align:center;">그룹</div></a>
                <a href="/append/person_popup" class="btn1-white btn1-space" onmouseover="this.style.backgroundColor='#F4F4F4';return true;" onmouseout="this.style.backgroundColor=''; return true;"><div style="width:80px;text-align:center;">개인</div></a>
              </div>
            ...
            <?php
                ...
                echo form_open('/append/group', $attributes);
            ?>
            <?php
            $data = array(
                'mobile_list' => '',
            ...
            <table border="0" class="basic">
                ...
                ?>
                    <tr class="font-13" bgcolor="#FFFFFF" onMouseOver="this.style.backgroundColor='#F4F4F4';return true;" onMouseOut="this.style.backgroundColor=''; return true;">
                        <td></td>
                        <td><input type="checkbox" name="chk_seq_no" value="<?=("{$row['gno']}|{$row['mobile_cnt']}")?>" /></td>
                        <td><a href="/append/person_popup/<?=$row['gno']?>"><?=$row['name']?></a></td>
                        <td><?=number_format($row['mobile_cnt'])?> 명</td>
                    </tr>
                <?php
                    }
                ?>
    ...
    <tr>
        <td align="center">
            <div class="btn1-group" style="text-align:center; margin-top:30px">
                <a id="export_string" onclick="addTargetMobile();" class="btn1-orange btn1-space"><div style="width:60px;text-align:center;">추가</div></a>
...
var addTargetMobile = function () {
    ...
    $("input[name=chk_seq_no]:checked").each(function() {
        var mobile_info = $(this).val();
        var arrayList = mobile_info.split("|");

        if (mobile_list != "") mobile_list += ",";
        mobile_list += arrayList[0];
        successCount += parseInt(arrayList[1]);
    });
    if (!successCount) {
        alert("추가할 목록을 선택하세요.");
        return;
    ...
    $.ajax({
        type: "POST",
        url: "/append/<?=($this->session->userdata('elect') === true ? 'elect_group' : 'group')?>",
        data: {
